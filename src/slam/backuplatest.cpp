...
#define THETA_RES 0.1
#define NUM_THETA_VALS 1
...
void SLAM::RotatePointCloud(vector<Vector2f> input,vector<Vector2f>& output, float angle)
{
...
void SLAM::TranslatePointCloud(vector<Vector2f> input,vector<Vector2f>& output, Vector2f translation)
{
  for(const auto& cur_point : input)
  {
...
void SLAM::ObserveLaser(const vector<float>& ranges,
  ...
  if(execute_csm_) {
    ...
    for(int ind_x=-NUM_X_VALS;ind_x<=NUM_X_VALS;ind_x++)
    {
      for(int ind_y=-NUM_Y_VALS;ind_y<=NUM_Y_VALS;ind_y++)
      {
        for(int ind_theta=-NUM_THETA_VALS;ind_theta<=NUM_THETA_VALS;ind_theta++)
        {
          ...
          for(auto cur_point : pointcloud_carframe)
          {
            // cout<<"initial x : "<<cur_point.x()<<", initial y : "<<cur_point.y()<<endl;
            Vector2f temp = Eigen::Rotation2Df(change_theta)*cur_point;
            // cout<<"rotated x : "<<temp.x()<<", rotated y : "<<temp.y()<<endl;
            temp = temp + changexy_prevcarframe;
            // cout<<"changed x : "<<temp.x()<<", changed y : "<<temp.y()<<endl;
            pointcloud_prevcarframe.push_back(temp);
            // exit(0);
          }
          float log_score_avg = raster_table_.lookup_pointcloud(pointcloud_prevcarframe);
    ...
    pointcloud_test = pointcloud_carframe;
    ...
    // }
    // printf("\n");
    for(int ind_x = -5;ind_x<=5;ind_x++)
    {
      // printf("%3d",ind_x);
      for(int ind_y = -5;ind_y<=5;ind_y++)
      {
        for(int ind_theta = -5;ind_theta<=5;ind_theta++)
        {
